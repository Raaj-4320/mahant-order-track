...
// Columns: supplier | pic+dim | product | marka | details | ctns | pcs/ctn | total pcs | rmb/pcs | line total | customer | action
export const LINE_GRID =
  "grid grid-cols-[minmax(0,0.38fr)_72px_72px_minmax(0,0.6fr)_minmax(0,0.6fr)_56px_76px_70px_76px_132px_minmax(0,0.5fr)_28px] items-center gap-1.5";
...
export function OrderLineRow({ line, onChange, onRemove, onUploadingChange, supplierSuggestions = [], customerSuggestions = [], customers = [] }: Props) {
  ...
  const topSupplierSuggestions = useMemo(
    () => supplierSuggestions.filter((name) => !supplierQuery || name.toLowerCase().includes(supplierQuery)).slice(0, 4),
  ...
  return (
    ...
    >
      <div className="relative"><Input compact value={line.supplierName ?? line.supplierSnapshot?.name ?? ""} onFocus={() => setSupplierOpen(true)} onBlur={() => window.setTimeout(() => setSupplierOpen(false), 120)} onChange={(e) => { onChange({ supplierName: e.target.value }); setSupplierOpen(true); }} placeholder="Supplier name" />{supplierOpen && topSupplierSuggestions.length > 0 ? <div className="absolute z-20 mt-1 w-full rounded-lg border border-border bg-bg-card shadow-card">{topSupplierSuggestions.map((name) => <button key={name} type="button" className="block w-full px-2 py-1 text-left text-[12px] hover:bg-bg-subtle" onMouseDown={(e)=>{e.preventDefault(); onChange({ supplierName: name }); setSupplierOpen(false);}}>{name}</button>)}</div> : null}</div>
