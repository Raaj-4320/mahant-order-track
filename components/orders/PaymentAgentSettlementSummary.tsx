    ...
    <div className="rounded border border-border/70 bg-bg-card p-3 text-[12px]">
      <div className="flex items-center justify-between gap-2 mb-2">
        ...
        <div className="flex items-center gap-1.5">
          <span className="rounded-full border border-border px-2 py-0.5 text-[10px] text-fg-subtle">Payment Agent: {paymentAgent.name} ({paymentAgent.agentCode})</span>
      ...
      <div className="grid grid-cols-1 gap-2 md:grid-cols-[1fr_1fr] md:gap-3">
        ...
        <div className="space-y-1.5">
          <div className="flex items-center justify-between gap-2"><label htmlFor="paid-now" className="text-fg-subtle">Paid Now to Payment Agent</label><input id="paid-now" type="number" min={0} value={paidNow} onChange={(e) => onPaidNowChange(Math.max(0, Number(e.target.value) || 0))} className="input h-7 w-[140px] text-right tabular-nums" /></div>
