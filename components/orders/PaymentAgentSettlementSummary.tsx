  ...
  return (
    <div className="rounded border border-border bg-bg-subtle p-3 space-y-2 text-[12px]">
      ...
      <div className="grid grid-cols-2 gap-x-3 gap-y-1">
        <div>Existing Credit</div><div className="text-right font-medium">{formatCNY(settlement.existingCredit)}</div>
        <div>Current Order Total</div><div className="text-right font-medium">{formatCNY(settlement.orderTotal)}</div>
        <div>Credit Used</div><div className="text-right font-medium">{formatCNY(settlement.creditUsed)}</div>
        <div>Payable Now</div><div className="text-right font-medium">{formatCNY(settlement.payableAfterCredit)}</div>
        ...
        <div>Remaining Payable</div><div className="text-right font-medium">{formatCNY(settlement.remainingPayable)}</div>
        <div>New Credit Created</div><div className="text-right font-medium">{formatCNY(settlement.newCreditCreated)}</div>
        <div>Resulting Agent Credit</div><div className="text-right font-semibold">{formatCNY(settlement.resultingCreditBalance)}</div>
      </div>
    </div>
  );
}
