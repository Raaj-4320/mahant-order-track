...
export const productFromFirestore = (doc: unknown): Product => {
  ...
  return {
    ...
    supplierName: asStr(p.supplierName) || undefined,
    ...
    discoveryImages: Array.isArray(p.discoveryImages) ? p.discoveryImages.filter((x): x is string => typeof x === "string" && x.trim().length > 0) : undefined,
    discoveryTotalCtns: asNum(p.discoveryTotalCtns),
    discoveryPcsPerCtn: asNum(p.discoveryPcsPerCtn),
    discoveryRate: asNum(p.discoveryRate),
...
export const productToFirestore = (entity: Product): Record<string, unknown> => ({
  ...
  discoveryImages: entity.discoveryImages ?? null,
  discoveryTotalCtns: entity.discoveryTotalCtns ?? null,
  discoveryPcsPerCtn: entity.discoveryPcsPerCtn ?? null,
  discoveryRate: entity.discoveryRate ?? null,
...
export const paymentAgentFromFirestore = (doc: unknown): PaymentAgent => {
  ...
  return {
    ...
    status: asStatus(p.status),
    totalOrdersPaid: asNum(p.totalOrdersPaid) ?? 0,
    ...
    totalPayableAmount: asNum(p.totalPayableAmount) ?? 0,
    currentDuePayable: asNum(p.currentDuePayable) ?? 0,
    totalUsedAmount: asNum(p.totalUsedAmount) ?? 0,
    currentPayable: asNum(p.currentPayable) ?? 0,
    notes: asStr(p.notes) || undefined,
    createdAt: asStr(p.createdAt, now),
    updatedAt: asStr(p.updatedAt, now),
...
export const paymentAgentToFirestore = (entity: PaymentAgent): Record<string, unknown> => sanitizeFirestorePayload({
  ...
  city: entity.city ?? null,
  notes: entity.notes ?? null,
  totalOrdersPaid: entity.totalOrdersPaid ?? 0,
  ...
  totalPayableAmount: entity.totalPayableAmount ?? 0,
  currentDuePayable: entity.currentDuePayable ?? 0,
  totalUsedAmount: entity.totalUsedAmount ?? 0,
  currentPayable: entity.currentPayable ?? entity.currentDuePayable ?? 0,
  lifecycle: entity.lifecycle ?? null,
}).value as Record<string, unknown>;
  ...
  const lines = Array.isArray(o.lines)
    ? o.lines.map((l) => {
        ...
        return seedDetailBoxesFromLegacy({
          ...
          loadingDate: typeof line.loadingDate === "string" ? asStr(line.loadingDate) : undefined,
...
export const orderToFirestore = (entity: Order): Record<string, unknown> => sanitizeFirestorePayload({
  ...
  lines: entity.lines.map((line, index) => ({
    ...
    loadingDate: line.loadingDate ?? null,
