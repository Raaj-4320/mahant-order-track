...
export const productFromOrderLine = async (order: Order, line: OrderLine, index: number): Promise<Product> => {
  ...
  const linePhoto = isHttpUrl(line.productPhotoUrl) ? line.productPhotoUrl : (process.env.NEXT_PUBLIC_PRODUCTS_DATA_SOURCE === "firebase" && isDataUrl(line.productPhotoUrl) ? "" : line.productPhotoUrl || "");
  return { id, productCode: existing?.productCode || code, sku: existing?.sku || code, name: line.details?.trim() || line.marka?.trim() || "Order Line Product", marka: line.marka?.trim() || existing?.marka || "", category: existing?.category || "Order Generated", unit: existing?.unit || "pcs", defaultDim: line.picDim?.trim() || existing?.defaultDim, photo: existing?.photo ? existing.photo : (linePhoto || ""), supplierId: line.supplierId || existing?.supplierId, supplierSnapshot: supplier ? { id: supplier.id, code: supplier.supplierCode, name: supplier.name } : existing?.supplierSnapshot, purchasePrice: existing?.purchasePrice, sellingPrice: Number(line.rmbPerPcs) || existing?.sellingPrice, defaultRmbPerPcs: Number(line.rmbPerPcs) || existing?.defaultRmbPerPcs, stockQty: lineTotalPcs(line), status: existing?.status || "active", createdAt: existing?.createdAt || now, updatedAt: now, source: existing?.source || "order-line", sourceOrderId: order.id, sourceOrderNumber: order.number || order.orderNumber, sourceLineId: line.id, sourceOrderIds: uniqueAppend(existing?.sourceOrderIds, order.id), sourceLineIds: uniqueAppend(existing?.sourceLineIds, `${order.id}:${line.id}`), catalogKey: existing?.catalogKey, generatedFromOrderLines: existing?.generatedFromOrderLines ?? true, lastSeenAt: now, lastLineTotalPcs: lineTotalPcs(line) };
};

export const syncOrderLinesToProducts = async (order: Order): Promise<{ synced: number; failed: number }> => {
  if (order.status !== "saved") return { synced: 0, failed: 0 };
  const service = getProductsService(); let synced = 0; let failed = 0;
  const lines = order.lines.filter(meaningfulLine);
  if (!lines.length) return { synced: 0, failed: 0 };
  for (let i = 0; i < lines.length; i++) { try { await service.upsertProduct(await productFromOrderLine(order, lines[i], i)); synced++; } catch { failed++; } }
