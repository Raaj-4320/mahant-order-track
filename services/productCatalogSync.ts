...
export const normalizeCatalogText = (value: string | undefined): string =>
  (value || "")
    .toLowerCase()
    .trim()
    .replace(/[\.,/#!$%\^&\*;:{}=\-_`~()\[\]"]/g, " ")
...
export const createCatalogKeyFromOrderLine = (_order: Order, line: OrderLine): string | null => {
  const supplierId = (line.supplierId || "").trim();
  const marka = normalizeCatalogText(line.marka);
  const details = normalizeCatalogText(line.details);
  if (!supplierId) return null;
  if (marka.length < 2 || details.length < 3) return null;
  return `${supplierId}|${marka}|${details}`;
};

export const createFallbackProductIdFromOrderLine = (order: Order, line: OrderLine) => `order-line-${order.id}-${line.id}`;

const hashCatalogKey = (key: string): string => {
  let h = 2166136261;
  for (let i = 0; i < key.length; i++) h = (h ^ key.charCodeAt(i)) * 16777619;
  return (h >>> 0).toString(36);
};

export const createProductIdFromCatalogKey = (catalogKey: string) => `catalog-${hashCatalogKey(catalogKey)}`;
...
export const productFromOrderLine = async (order: Order, line: OrderLine, index: number): Promise<Product> => {
  ...
  const catalogKey = createCatalogKeyFromOrderLine(order, line);
  const id = catalogKey ? createProductIdFromCatalogKey(catalogKey) : createFallbackProductIdFromOrderLine(order, line);
