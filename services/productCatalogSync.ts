...
export const syncOrderLinesToProducts = async (order: Order): Promise<{ synced: number; failed: number }> => {
  ...
  for (let i = 0; i < order.lines.length; i++) { try { await service.upsertProduct(await productFromOrderLine(order, order.lines[i], i)); synced++; } catch { failed++; } }
  return { synced, failed };
};

export const archiveProductsForRemovedOrderLines = async (orderId: string, removedLineIds: string[]) => {
  const service = getProductsService();
  for (const lineId of removedLineIds) await service.archiveProduct(`order-line-${orderId}-${lineId}`);
};
