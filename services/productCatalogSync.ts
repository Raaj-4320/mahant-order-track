...
export const syncOrderLinesToProducts = async (order: Order): Promise<{ synced: number; failed: number; failures: ProductSyncFailure[] }> => {
  ...
  for (let i = 0; i < order.lines.length; i++) {
    ...
    try {
      ...
      logProduct("upsert_generated_product_start", { lineId: line.id, generatedProductId: payload.id });
      ...
      logError("upsert_generated_product_failure", { lineId: line.id, generatedProductId, errorCode, errorMessage });
    }
  }
