  ...
  for (const [agentId, events] of eventGroups.entries()) {
    ...
    const replacementSplit: PaymentAgentOrderSplit = {
      ...
      settlementSnapshot: {
        ...
        existingCredit: clamp(primarySplit?.settlementSnapshot?.existingCredit ?? 0),
        creditUsed,
        payableAfterCredit: remainingPayable,
        remainingPayable,
        newCreditCreated: clamp(primarySplit?.settlementSnapshot?.newCreditCreated ?? 0),
        resultingCreditBalance: Math.max(0, clamp(primarySplit?.settlementSnapshot?.existingCredit ?? 0) - creditUsed),
        paidNow: creditUsed,
