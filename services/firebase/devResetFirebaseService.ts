...
export async function deleteEverythingForBusiness(options?: { includeSettings?: boolean }): Promise<{ orders: number; products: number; paymentAgents: number; paymentAgentLedger: number; customers: number; settings?: number; }> {
  ...
  } as { orders: number; products: number; paymentAgents: number; paymentAgentLedger: number; customers: number; settings?: number; };
