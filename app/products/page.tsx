...
export default function ProductsPage() {
  ...
  const lowStock = rows.filter((r) => r.isLowStock && r.status === "active").length;
  const activeCatalogValue = rows.filter((r) => r.status === "active").reduce((s, r) => s + (r.catalogValue ?? 0), 0);
  ...
  useEffect(() => {
    ...
    logDataFlow("Products", { functionsCalled: ["useProducts.reload", "productsService.listProducts"], dbPaths: ["businesses/{businessId}/products"], result: { count: rows.length, reachedComponent: true, renderedRows: filtered.length }, counts: { totalProducts: rows.length, activeProducts: active, inactiveProducts: inactive, visibleProducts: filtered.length, selectedStatusFilter: status }, activeCatalogValue, sampleProducts: filtered.slice(0,5).map((p) => ({ id: p.id, sku: p.sku, name: p.name, status: p.status })), visibleActionsSummary: ["Add Product", "Edit Product", "Archive Product"] });
  }, [isProductsLoading, error, rows.length, filtered.length, active, inactive, status, activeCatalogValue]);
