  ...
  return (<PageShell title="Products"><div className="space-y-4 p-6">
    ...
    {open && <div className="fixed inset-0 z-50 bg-black/40 grid place-items-center p-4"><div className="card w-full max-w-3xl p-4 space-y-3"><div className="text-lg font-semibold">{form.id ? "Edit Product" : "Add Product"}</div>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
        ...
        <div className="space-y-1"><Input type="file" accept="image/*" onChange={(e) => setFile(e.target.files?.[0] ?? null)} />{form.photo && <button type="button" title="Open image preview" aria-label="Open image preview" className="cursor-zoom-in" onClick={() => setPreviewImage(form.photo)}><img src={form.photo} alt="preview" className="h-14 w-14 object-cover rounded border" /></button>}</div>
      </div>
