...
export default function ProductsPage() {
  ...
  const [q, setQ] = useState(""); const [status, setStatus] = useState("all"); const [category, setCategory] = useState("all");
  ...
  return (<PageShell title="Products"><div className="space-y-4 p-6">
    ...
    {open && <div className="fixed inset-0 z-50 bg-black/40 grid place-items-center p-4"><div className="card w-full max-w-3xl p-4 space-y-3"><div className="text-lg font-semibold">{form.id ? "Edit Product" : "Add Product"}</div>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
        ...
        <Input type="number" value={form.sellingPrice ?? ""} onChange={(e) => setForm((s) => ({ ...s, sellingPrice: e.target.value === "" ? undefined : Number(e.target.value), defaultRmbPerPcs: e.target.value === "" ? undefined : Number(e.target.value) }))} placeholder="Selling Price / RMB per PCS" />
