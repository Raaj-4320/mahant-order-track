...
export default function PaymentAgentsPage() {
  ...
  const filtered = useMemo(() => scopeRows.filter((r) => [
    r.agent.name,
    r.agent.agentCode,
    r.agent.wechatId || "",
    r.agent.phone || "",
    ...
    r.agent.notes || "",
    r.agent.status,
    formatAmount(r.currentCredit),
    formatAmount(r.currentDuePayable),
    formatAmount(r.totalPaidAmount),
    formatAmount(r.totalOrderAmount),
  ].join(" ").toLowerCase().includes(q.toLowerCase().trim())), [scopeRows, q]);
  ...
  return <PageShell title="Payment Agents"><div className="space-y-4 p-6">
    ...
    <div className="card p-3 flex flex-wrap gap-2 items-center"><div className="min-w-[280px] flex-1"><Input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search by agent, WeChat, phone, country, status, credit, due..." leadingIcon={<Search size={14} />} /></div><div className="w-[160px]"><Select value={status} onChange={(e) => setStatus(e.target.value)} options={[{ value: "all", label: "All Statuses" }, { value: "active", label: "Active" }, { value: "inactive", label: "Inactive" }]} /></div><Button size="sm" variant="secondary" disabled title="Filtering is not enabled in this phase."><Filter size={14} />More</Button><Button size="sm" variant="secondary" disabled title="Export is not enabled in this phase."><Download size={14} />Export</Button></div>

    <div className="card overflow-hidden"><div className="overflow-x-auto"><table className="w-full min-w-[1220px] text-[13px]"><thead className="bg-bg-subtle"><tr><th className="px-4 py-2 text-left">Agent</th><th>Opening Credit</th><th>Current Credit</th><th>Total Order</th><th>Total Paid</th><th>Current Due</th><th>Status</th><th className="w-[360px] px-4 text-right">Actions</th></tr></thead><tbody>{filtered.map((r) => { const ledgerTable = buildLedgerTable(r.agent.id, r.agent.openingCreditBalance ?? 0); return <><tr key={r.agent.id} className="border-t border-border"><td className="px-4 py-3"><div className="font-semibold">{r.agent.name}</div><div className="text-[11.5px] text-fg-subtle">{r.agent.agentCode}</div></td><td>{formatAmount(r.agent.openingCreditBalance ?? 0)}</td><td>{formatAmount(r.currentCredit)}</td><td>{formatAmount(r.totalOrderAmount)}</td><td>{formatAmount(r.totalPaidAmount)}</td><td>{formatAmount(r.currentDuePayable)}</td><td><StatusBadge status={r.agent.status} /></td><td className="px-4 py-3 text-right align-middle"><div className="flex flex-wrap justify-end gap-2"><Button size="sm" variant="secondary" onClick={() => setPayAgentId(r.agent.id)}>+ Payment</Button><Button size="sm" variant="secondary" onClick={() => toggleLedger(r.agent.id)}>View Ledger</Button><Button size="sm" variant="secondary" onClick={() => startEdit(r.agent)}>Edit</Button><Button size="sm" variant="secondary" onClick={() => removePaymentAgent(r.agent, r.currentDuePayable, r.currentCredit, r.totalOrders)} title="Delete payment agent if due/credit are zero and no order/ledger history exists.">Delete</Button></div></td></tr>
