...
export default function PaymentAgentsPage() {
  ...
  useEffect(() => {
    console.log("[PAYMENT_AGENTS_RUNTIME_VERSION]", {
      component: "app/payment-agents/page.tsx",
      actionLabel: "Delete",
      commitHint: "runtime-debug-delete-label",
  ...
  const removePaymentAgent = async (agent: PaymentAgent, currentDue: number, currentCredit: number, totalOrders: number) => {
    if (currentDue > 0) {
      logUI("payment_agent_delete_blocked", { agentId: agent.id, reason: "current_due", currentDue });
      return pushToast({ tone: "info", text: "Payment agent has payable due. Clear due before deleting." });
    }
    if (currentCredit > 0) {
      logUI("payment_agent_delete_blocked", { agentId: agent.id, reason: "current_credit", currentCredit });
      return pushToast({ tone: "info", text: "Payment agent has credit balance. Clear credit before deleting." });
    }
    ...
    if (totalOrders > 0 || linkedSavedOrdersCount > 0 || ledgerHistoryCount > 0) {
      logUI("payment_agent_delete_blocked", { agentId, reason: "order_or_ledger_history", totalOrders, linkedSavedOrdersCount, ledgerHistoryCount });
      return pushToast({ tone: "info", text: "Payment agent has order/ledger history and cannot be deleted safely." });
    }
    if (!confirm(`Delete payment agent ${agent.name}? This cannot be undone.`)) return;
    logUI("payment_agent_delete_started", { agentId, status: agent.status });
    try {
      await deletePaymentAgent(agentId);
      logUI("payment_agent_delete_success", { agentId });
      pushToast({ tone: "success", text: `Payment agent ${agent.name} deleted.` });
    } catch (e) {
      logUI("payment_agent_delete_failed", { agentId, error: e instanceof Error ? e.message : String(e) });
  ...
  return <PageShell title="Payment Agents"><div className="space-y-4 p-6">
    <div className="flex items-center justify-between"><div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-5 flex-1"><StatCard label="Total Agents" value={rows.length.toString()} icon={<Wallet size={16} />} /><StatCard label="Active Agents" value={active.toString()} /><StatCard label="Total Order Amount" value={formatAmount(rows.reduce((s, r) => s + r.totalOrderAmount, 0))} /><StatCard label="Total Paid" value={formatAmount(rows.reduce((s, r) => s + r.totalPaidAmount, 0))} /><StatCard label="Current Due" value={formatAmount(rows.reduce((s, r) => s + r.currentDuePayable, 0))} /></div><Button onClick={() => { setForm({ id: "", name: "", agentCode: "", phone: "", wechatId: "", country: "", openingCredit: "", notes: "", status: "active" }); setOpen(true); }} variant="primary" className="ml-3"><Plus size={14} />Add Payment Agent</Button></div>
