...
export default function PaymentAgentsPage() {
  ...
  const removePaymentAgent = async (agent: PaymentAgent, currentDue: number, currentCredit: number, totalOrders: number) => {
    ...
    if (riskDetected) {
      ...
      const warningLines = [
        "Delete payment agent with payable/credit/history records?",
        "",
        `Agent: ${agent.name || agentId}`,
        `Current Due: ${formatAmount(currentDue)}`,
        `Current Credit: ${formatAmount(currentCredit)}`,
        `Order history count: ${Math.max(totalOrders, linkedSavedOrdersCount)}`,
        `Ledger history count: ${ledgerHistoryCount}`,
        "",
        "Deleting this payment agent will remove the agent record only. Existing orders and ledger entries will remain for audit history.",
        "",
        "Type DELETE AGENT to continue.",
      ];
      logUI("payment_agent_delete_force_confirm_opened", riskPayload);
      const typed = prompt(warningLines.join("\n"), "");
      if (typed !== "DELETE AGENT") {
        logUI("payment_agent_delete_force_cancelled", { agentId, typedValuePresent: Boolean(typed) });
        return;
      }
      logUI("payment_agent_delete_force_confirmed", riskPayload);
    } else {
    ...
    try {
      ...
      pushToast({ tone: "success", text: riskDetected ? "Payment agent deleted. Historical orders and ledger entries were kept." : `Payment agent ${agent.name} deleted.` });
    } catch (e) {
