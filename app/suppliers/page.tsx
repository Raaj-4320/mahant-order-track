...
export default function SuppliersPage() {
  ...
  useEffect(() => { if (ordersLoading) return; logDataFlow("Suppliers", { functionsCalled:["useOrders.reload","useSuppliers.reload"], dbPaths:["businesses/{businessId}/orders"], result:{reachedComponent:true}, counts:{wechatGroups:filteredWechat.length,uniqueSupplierGroups:filteredUnique.length}, samples:{wechat:filteredWechat.slice(0,5).map((g)=>({wechatId:g.wechatId,totalOrders:g.totalOrders})), unique:filteredUnique.slice(0,5).map((g)=>({supplier:g.supplierName,totalOrders:g.totalOrders}))} }); }, [ordersLoading, filteredWechat.length, filteredUnique.length]);
