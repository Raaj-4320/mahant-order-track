...
export default function CustomersPage() {
  ...
  const { data: customers, isLoading, error, recordPaymentToCustomer, archiveCustomer, reload } = useCustomers();
  ...
  const archive = async (customerId: string) => {
    ...
    if (currentReceivable > 0) {
      logUI("customer_archive_blocked", { customerId, reason: "outstanding_receivable", currentReceivable });
      return pushToast({ tone: "info", text: "Customer has outstanding receivable. Receive payment or clear balance before archiving." });
    }
    if (storeCredit > 0) {
      logUI("customer_archive_blocked", { customerId, reason: "store_credit", storeCredit });
      return pushToast({ tone: "info", text: "Customer has store credit. Clear or settle credit before archiving." });
    }
    if (!confirm(`Archive customer ${customer.displayName}?`)) return;
    logUI("customer_archive_started", { customerId, status: customer.status });
    try {
      await archiveCustomer(customerId);
      logUI("customer_archive_success", { customerId });
      pushToast({ tone: "success", text: `Customer ${customer.displayName} archived.` });
    } catch (e) {
      logUI("customer_archive_failed", { customerId, error: e instanceof Error ? e.message : String(e) });
      pushToast({ tone: "danger", text: e instanceof Error ? e.message : "Could not archive customer." });
    }
  };
