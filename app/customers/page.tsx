...
export default function CustomersPage() {
  ...
  const removeCustomer = async (customerId: string) => {
    ...
    if (riskDetected) {
      ...
      const warningLines = [
        "Delete customer with financial/history records?",
        "",
        `Customer: ${customer.displayName || customer.name || customerId}`,
        `Current Receivable: ${formatAmount(currentReceivable)}`,
        `Store Credit: ${formatAmount(storeCredit)}`,
        `Order history count: ${Math.max(totalOrders, orderHistoryCount)}`,
        `Ledger history count: ${ledgerHistoryCount}`,
        "",
        "Deleting this customer will remove the customer record only. Existing orders and ledger entries will remain for audit history.",
        "",
        "Type DELETE CUSTOMER to continue.",
      ];
      logUI("customer_delete_force_confirm_opened", riskPayload);
      const typed = prompt(warningLines.join("\n"), "");
      if (typed !== "DELETE CUSTOMER") {
        logUI("customer_delete_force_cancelled", { customerId, typedValuePresent: Boolean(typed) });
        return;
      }
      logUI("customer_delete_force_confirmed", riskPayload);
      console.log("[CUSTOMER_DELETE_TRACE] final_decision", JSON.stringify({ allowed: true, forced: true, ...riskPayload }, null, 2));
    } else {
      console.log("[CUSTOMER_DELETE_TRACE] final_decision", JSON.stringify({ allowed: true, forced: false, blockReason: null, userMessage: null, customerId }, null, 2));
    ...
    try {
      ...
      pushToast({ tone: "success", text: riskDetected ? "Customer deleted. Historical orders and ledger entries were kept." : `Customer ${customer.displayName} deleted.` });
    } catch (e) {
