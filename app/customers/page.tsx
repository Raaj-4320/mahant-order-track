...
export default function CustomersPage() {
  ...
  const removeCustomer = async (customerId: string) => {
    ...
    if (currentReceivable > 0) {
      const blockPayload = { customerId, reason: "outstanding_receivable", currentReceivable };
      console.log("[CUSTOMER_DELETE_TRACE] final_decision", JSON.stringify({ allowed: false, blockReason: "outstanding_receivable", userMessage: "Customer has outstanding receivable. Clear balance before deleting.", ...blockPayload }, null, 2));
      logUI("customer_delete_blocked", blockPayload);
      return pushToast({ tone: "info", text: "Customer has outstanding receivable. Clear balance before deleting." });
    }
    if (storeCredit > 0) {
      const blockPayload = { customerId, reason: "store_credit", storeCredit };
      console.log("[CUSTOMER_DELETE_TRACE] final_decision", JSON.stringify({ allowed: false, blockReason: "store_credit", userMessage: "Customer has store credit. Clear or settle credit before deleting.", ...blockPayload }, null, 2));
      logUI("customer_delete_blocked", blockPayload);
      return pushToast({ tone: "info", text: "Customer has store credit. Clear or settle credit before deleting." });
    }
    ...
    if (totalOrders > 0 || orderHistoryCount > 0 || ledgerHistoryCount > 0) {
      const blockPayload = { customerId, reason: "order_or_ledger_history", totalOrders, orderHistoryCount, ledgerHistoryCount };
      console.log("[CUSTOMER_DELETE_TRACE] final_decision", JSON.stringify({ allowed: false, blockReason: "order_or_ledger_history", userMessage: "Customer has order/ledger history and cannot be deleted safely.", ...blockPayload }, null, 2));
      logUI("customer_delete_blocked", blockPayload);
      return pushToast({ tone: "info", text: "Customer has order/ledger history and cannot be deleted safely." });
    }
    console.log("[CUSTOMER_DELETE_TRACE] final_decision", JSON.stringify({ allowed: true, blockReason: null, userMessage: null, customerId }, null, 2));
    console.log("[CUSTOMER_DELETE_TRACE] confirm_opened", JSON.stringify({ customerId, customerName: customer.displayName || customer.name }, null, 2));
    const confirmed = confirm(`Delete customer ${customer.displayName}? This cannot be undone.`);
    console.log("[CUSTOMER_DELETE_TRACE] confirm_result", JSON.stringify({ customerId, confirmed }, null, 2));
    if (!confirmed) return;
    logUI("customer_delete_started", JSON.parse(JSON.stringify({ customerId, status: customer.status }, null, 2)));
    try {
      console.log("[CUSTOMER_DELETE_TRACE] service_delete_start", JSON.stringify({ customerId, source: process.env.NEXT_PUBLIC_CUSTOMERS_DATA_SOURCE ?? (process.env.NEXT_PUBLIC_ORDERS_DATA_SOURCE ?? "mock") }, null, 2));
      await deleteCustomer(customerId);
      logUI("customer_delete_success", JSON.parse(JSON.stringify({ customerId }, null, 2)));
      pushToast({ tone: "success", text: `Customer ${customer.displayName} deleted.` });
    } catch (e) {
      logUI("customer_delete_failed", JSON.parse(JSON.stringify({ customerId, error: e instanceof Error ? e.message : String(e) }, null, 2)));
