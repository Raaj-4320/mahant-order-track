...
export default function CustomersPage() {
  ...
  const removeCustomer = async (customerId: string) => {
    ...
    if (currentReceivable > 0) {
      logUI("customer_delete_blocked", { customerId, reason: "outstanding_receivable", currentReceivable });
      return pushToast({ tone: "info", text: "Customer has outstanding receivable. Clear balance before deleting." });
    }
    if (storeCredit > 0) {
      logUI("customer_delete_blocked", { customerId, reason: "store_credit", storeCredit });
    ...
    const orderHistoryCount = sourceOrders
    ...
    try { ledgerHistoryCount = (await customerLedgerService.listCustomerLedgerEntries(customerId)).length; } catch {}
    if (getCustomerTotalOrders(customer) > 0 || orderHistoryCount > 0 || ledgerHistoryCount > 0) {
      logUI("customer_delete_blocked", { customerId, reason: "order_or_ledger_history", totalOrders: getCustomerTotalOrders(customer), orderHistoryCount, ledgerHistoryCount });
      return pushToast({ tone: "info", text: "Customer has order/ledger history and cannot be deleted safely." });
    }
    if (!confirm(`Delete customer ${customer.displayName}? This cannot be undone.`)) return;
    logUI("customer_delete_started", { customerId, status: customer.status });
    try {
      await deleteCustomer(customerId);
      logUI("customer_delete_success", { customerId });
      pushToast({ tone: "success", text: `Customer ${customer.displayName} deleted.` });
    } catch (e) {
      logUI("customer_delete_failed", { customerId, error: e instanceof Error ? e.message : String(e) });
