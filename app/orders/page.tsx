...
export default function OrdersPage() {
  ...
  useEffect(() => {
    logOrder("orders_load_start", { source: isFirebaseOrdersMode ? "firebase" : "mock" });
  ...
  useEffect(() => {
    ...
    logOrder("orders_load_success", {
      source: isFirebaseOrdersMode ? "firebase" : "mock",
      orderCount: allOrders.length,
      savedCount: allOrders.filter((o) => o.status === "saved").length,
      draftCount: allOrders.filter((o) => o.status === "draft").length,
      archivedCount: allOrders.filter((o) => o.status === "archived").length,
      totalAmount: allOrders.reduce((sum, o) => sum + orderTotal(o), 0),
      orders: allOrders.slice(0, 5).map(summarizeOrderForLog),
    });
  }, [isFirebaseOrdersMode, isOrdersLoading, ordersLoadError, firebaseOrders, orders]);

  useEffect(() => {
    logOrder("order_history_rendered", {
      visibleCount: history.length,
      query: query.trim() || undefined,
      visibleOrders: history.slice(0, 5).map(summarizeOrderForLog),
    });
  }, [history, query]);
  ...
  const onSave = async (status: Order["status"]) => {
    ...
    logOrder("save_order_complete", { orderId: savedOrder.id, upserted: true, productSyncFailed: Boolean(result.failed), productSyncFailures: result.failures, settlementApplied: isFirebaseOrdersMode, receivablesApplied: true });
