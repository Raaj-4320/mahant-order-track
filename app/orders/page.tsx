...
export default function OrdersPage() {
  ...
  const history = useMemo(() => activeOrders.filter((o) => { const q=query.toLowerCase().trim(); if(!q) return true; const supplierText=o.lines.map(l=>suppliers.find(s=>s.id===l.supplierId)?.name??"").join(" ").toLowerCase(); const customerText=o.lines.map(l=>customers.find(c=>c.id===l.customerId)?.name??"").join(" ").toLowerCase(); const payment=paymentAgents.find(p=>p.id===o.paymentBy)?.name.toLowerCase()??""; return o.number.toLowerCase().includes(q)||o.wechatId.toLowerCase().includes(q)||supplierText.includes(q)||customerText.includes(q)||payment.includes(q); }).slice(0, 10), [activeOrders, query, paymentAgents]);
  ...
  const supplierSuggestions = useMemo(() => {
    const fromOrders = activeOrders.flatMap((o) => o.lines.map((l) => (l.supplierName?.trim() || suppliers.find((s) => s.id === l.supplierId)?.name || "").trim()));
