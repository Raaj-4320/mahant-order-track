...
export default function OrdersPage() {
  ...
  const onSave = async (status: Order["status"]) => {
    ...
    const nextOrders = orders.some((o) => o.id === savedOrder.id) ? orders.map((o) => (o.id === savedOrder.id ? savedOrder : o)) : [savedOrder, ...orders];
    ...
    await recalculateFromOrders(nextOrders);
    ...
    setEditingOrderId(null); setRemovedLineIds([]); setOriginalLineIds(new Set()); setDraft(createEmptyDraft(orders, paymentAgents[0]?.id || ""));
  };

  const onCancel = () => { setEditingOrderId(null); setRemovedLineIds([]); setOriginalLineIds(new Set()); setDraft(createEmptyDraft(orders, paymentAgents[0]?.id || "")); pushToast({ tone: "info", text: "Draft reset to new order." }); };
  ...
  const removeOrder = async (o: Order) => {
    ...
    deleteOrder(o.id);
    await recalculateFromOrders(orders.filter((x) => x.id !== o.id));
