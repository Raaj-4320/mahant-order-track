...
export default function OrdersPage() {
  ...
  const { data: paymentAgents, recalculateFromOrders } = usePaymentAgents();
  const { draftOrders: firebaseDraftOrders, autosaveDraft } = useOrders();
  ...
  const onSave = async (status: Order["status"]) => {
    ...
    const mergedOrders = orders.some((o) => o.id === savedOrder.id) ? orders.map((o) => (o.id === savedOrder.id ? savedOrder : o)) : [savedOrder, ...orders];
  ...
  const startAdd = () => { setEditingOrderId(null); setRemovedLineIds([]); setOriginalLineIds(new Set()); setDraft(createEmptyDraft(orders, "")); setMode("add"); };
  const drafts = useMemo(() => (ordersDataSource === "firebase" ? firebaseDraftOrders : orders.filter((o) => o.status === "draft")), [orders, firebaseDraftOrders, ordersDataSource]);
  ...
  const autosaveStatus = useDraftAutosave({ enabled: ordersDataSource === "firebase" && (mode === "add" || mode === "edit"), draft, activeUploads, autosaveDraft, onSaved: (saved) => setDraft((d) => ({ ...d, id: saved.id })) });
  ...
  return (
    <div className="flex h-screen min-h-0 flex-col">
      ...
      <main className="min-h-0 flex-1 overflow-y-auto p-4 space-y-4">
        {(mode === "add" || mode === "edit") && <>
          {ordersDataSource === "firebase" && <div className="text-[11px] text-fg-subtle px-5">{autosaveStatus === "saving" ? "Saving draft..." : autosaveStatus === "saved" ? "Draft saved" : autosaveStatus === "error" ? "Draft autosave failed" : ""}</div>}
