    ...
    <div className="flex h-screen min-h-0 flex-col">
      ...
      {isOrderModalOpen && <div className="fixed inset-0 z-50 bg-black/45 p-3 md:p-6" onClick={onCancel}>
        <div className="relative mx-auto w-full max-w-[1400px] h-[90vh] rounded-2xl border border-border bg-bg-card shadow-card flex flex-col overflow-hidden" onClick={(e) => e.stopPropagation()}>
          ...
          {validationWarning.visible ? <div className="absolute left-1/2 top-5 z-[70] w-[92%] max-w-[520px] -translate-x-1/2 rounded-xl border border-amber-400/50 bg-amber-500/15 p-3 shadow-card">
            <div className="flex items-start justify-between gap-2">
              <div>
                <div className="text-sm font-semibold text-amber-200">Missing before Save Order</div>
                <ul className="mt-1 list-disc pl-5 text-[12px] text-amber-100/95 space-y-0.5">{validationWarning.items.map((item, idx) => <li key={`${item}-${idx}`}>{item}</li>)}</ul>
              </div>
              <button className="text-amber-100 text-xs" onClick={() => setValidationWarning({ visible: false, items: [] })}>✕</button>
            </div>
          </div> : null}
