...
import { ensureFinalOrderNumber } from "@/services/orderNumberService";
...
export default function OrdersPage() {
  ...
  const { data: firebaseOrders, isLoading: isOrdersLoading, error: ordersLoadError, draftOrders: firebaseDraftOrders, autosaveDraft, upsertOrder: upsertFirebaseOrder, archiveOrder: archiveFirebaseOrder, reload: reloadFirebaseOrders, allocateNextOrderNumber } = useOrders();
  ...
  const onSave = async (status: Order["status"]) => {
    logDataFlow("Orders", JSON.stringify({ event: status === "draft" ? "draft_save_started" : "order_save_started", status, lineCount: draft.lines.length, orderNumber: draft.number }, null, 2));
    ...
    if (status === "draft") {
      ...
      setMode("history");
      logDataFlow("Orders", JSON.stringify({ event: "draft_save_completed", orderId: draftOrder.id, orderNumber: draftOrder.number || draftOrder.orderNumber }, null, 2));
    ...
    logDataFlow("Orders", JSON.stringify({ event: "order_save_completed", orderSaved: true, productsSynced: !Boolean(result.failed), customerResolved: true, customerReceivablesApplied: true, paymentSettlementApplied: isFirebaseOrdersMode }, null, 2));
  ...
  const startAdd = async () => {
    ...
    if (isFirebaseOrdersMode && firebaseDraftOrders.length) {
      ...
      try {
        const reserved = await allocateNextOrderNumber();
        setDraft(createEmptyDraft(orders, "", reserved));
      } catch (e) {
        ...
        <section className="card p-4">
          ...
          <div className="space-y-2">{history.length === 0 ? <div className="text-[12px] text-fg-subtle">No orders yet. Click Add Order to create one.</div> : history.map((o) => (
            <div key={o.id} className="rounded border border-border p-3 flex items-center justify-between gap-2">
              <div className="text-[12px]"><div className="font-semibold">{o.number || o.orderNumber || "—"}</div><div className="text-fg-subtle">{formatDate(o.date)} · {paymentAgents.find((p) => p.id === o.paymentBy)?.name ?? "—"} · {o.wechatId || "—"}</div><div className="text-fg-subtle">{o.lines.length} lines · {formatAmount(orderTotal(o))}</div></div>
