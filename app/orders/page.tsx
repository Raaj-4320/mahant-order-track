...
export default function OrdersPage() {
  ...
  const onSave = async (status: Order["status"]) => {
    ...
    const savedOrder = { ...draft, status: "saved" as const };
  ...
  return (
    <div className="flex h-screen min-h-0 flex-col">
      ...
      <main className="min-h-0 flex-1 overflow-y-auto p-4 space-y-4">
        ...
        <OrderForm draft={draft} setDraft={(u) => setDraft((d) => u(d))} onUploadingChange={onUploadingChange} onRemoveLine={handleRemoveLine} />
        <OrderFooter total={total} onCancel={onCancel} onSaveDraft={() => onSave("draft")} onSaveOrder={() => onSave("saved")} saveOrderLabel={editingOrderId ? "Save Changes" : "Save Order"} />
