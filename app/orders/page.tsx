...
type HoverPreview = { key: string; anchor: HTMLElement | null; lines: Array<{ line: any; idx: number }> } | null;
  ...
  return (
    <div className="flex h-screen min-h-0 flex-col">
                  ...
                  return <tr key={o.id} className="border-t border-border/80 hover:bg-bg-subtle/40 align-middle h-[68px]">
                    ...
                    <td>
                      <div className="flex items-center gap-1.5">
                        {primary.map(({ line, idx, image }) => <button key={line.id || idx} type="button" onMouseEnter={(e) => handlePreviewAnchorEnter({ key: `${o.id}-${idx}`, anchor: e.currentTarget, lines: [{ line, idx }] })} onMouseLeave={handlePreviewAnchorLeave} onFocus={(e) => handlePreviewAnchorEnter({ key: `${o.id}-${idx}`, anchor: e.currentTarget, lines: [{ line, idx }] })} onBlur={handlePreviewAnchorLeave} onClick={() => image && setPreviewImage({ src: image, alt: "Order history product photo" })} title={image ? "Open image preview" : undefined} aria-label={image ? "Open image preview" : undefined} className="grid h-10 w-10 place-items-center overflow-hidden rounded-lg border border-border bg-bg-subtle">{image ? <img src={getCloudinaryOptimizedUrl(image, { width: 96, height: 96, crop: "fill" })} alt="product" className="h-full w-full cursor-zoom-in object-cover" loading="lazy" decoding="async" /> : <span className="text-[10px] text-fg-subtle">—</span>}</button>)}
      ...
      <FloatingPortal anchorRef={{ current: hoverPreview?.anchor as any }} open={Boolean(hoverPreview?.anchor)} width={420} placement="bottom" align="center">
        <div className="rounded-[22px] border border-border bg-bg-card p-4 shadow-card" onMouseEnter={() => { hoverPopupActiveRef.current = true; if (hoverCloseTimerRef.current) window.clearTimeout(hoverCloseTimerRef.current); }} onMouseLeave={() => { hoverPopupActiveRef.current = false; scheduleHoverPreviewClose(); }}>
          <div className="max-h-[74vh] space-y-4 overflow-y-auto pr-1 text-[12px]">
            {(hoverPreview?.lines || []).map(({ line, idx }) => {
              ...
              return <div key={`${line.id}-${idx}`} className="rounded-2xl border border-border/90 bg-bg p-4">
                <div className="relative mb-4 flex min-h-[182px] items-end rounded-2xl border border-border bg-bg-subtle/60 p-3">
                  <button type="button" className="grid h-36 w-44 place-items-center overflow-hidden rounded-xl border border-border bg-bg-subtle text-[11px] text-fg-subtle cursor-zoom-in" title="Open image preview" aria-label="Open image preview" onClick={(e) => { e.stopPropagation(); product && setPreviewImage({ src: product, alt: "Product photo" }); }}>{product ? <img src={getCloudinaryOptimizedUrl(product, { width: 340, height: 280, crop: "fill" })} alt="product" className="h-full w-full object-cover" loading="lazy" decoding="async" /> : "Not present"}</button>
                  <button type="button" className="absolute bottom-3 right-3 grid h-24 w-24 place-items-center overflow-hidden rounded-xl border border-border bg-bg-subtle text-[11px] text-fg-subtle cursor-zoom-in" title="Open image preview" aria-label="Open image preview" onClick={(e) => { e.stopPropagation(); dim && setPreviewImage({ src: dim, alt: "Dimension photo" }); }}>{dim ? <img src={getCloudinaryOptimizedUrl(dim, { width: 220, height: 220, crop: "fill" })} alt="dimension" className="h-full w-full object-cover" loading="lazy" decoding="async" /> : "Not present"}</button>
                </div>
                <div className="space-y-3">
                  <div><div className="text-[10px] font-semibold uppercase tracking-[0.14em] text-fg-subtle">Marka</div><div className="text-[24px] font-bold leading-tight text-fg">{line.marka?.trim() || "Not present"}</div></div>
                  <div><div className="text-[10px] font-semibold uppercase tracking-[0.14em] text-fg-subtle">Details</div><div className="text-[20px] font-medium leading-tight text-fg-muted">{line.details?.trim() || "Not present"}</div></div>
                  <div className="grid grid-cols-3 divide-x divide-border rounded-xl border border-border bg-bg-subtle/70">
                    <div className="px-3 py-2"><div className="text-[10px] text-fg-subtle">Total Ctns</div><div className="text-[15px] font-semibold">{ctns > 0 ? ctns.toLocaleString() : "—"}</div></div>
                    <div className="px-3 py-2"><div className="text-[10px] text-fg-subtle">Pcs/Ctn</div><div className="text-[15px] font-semibold">{pcsPer > 0 ? pcsPer.toLocaleString() : "—"}</div></div>
                    <div className="px-3 py-2"><div className="text-[10px] text-fg-subtle">Total Pcs</div><div className="text-[15px] font-semibold">{totalPcs > 0 ? totalPcs.toLocaleString() : "—"}</div></div>
                  </div>
                  <div className="grid grid-cols-2 divide-x divide-border rounded-xl border border-border bg-bg-subtle/70">
                    <div className="px-3 py-2"><div className="text-[10px] text-fg-subtle">Rate/Pcs</div><div className="text-[15px] font-semibold">{asMetric(rate)}</div></div>
                    <div className="px-3 py-2"><div className="text-[10px] text-fg-subtle">Total Amount</div><div className="text-[15px] font-semibold">{asMetric(totalAmount)}</div></div>
                  </div>
                </div>
              </div>;
            })}
