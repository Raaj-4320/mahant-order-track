...
export default function OrdersPage() {
  ...
  const customerSuggestions = useMemo(() => {
    ...
    const fromOrders = activeOrders.flatMap((o) => o.lines.map((l) => l.customerId?.trim())).filter(Boolean) as string[];
  ...
  const onSave = async (status: Order["status"]) => {
    ...
    const existingEntries: Array<[string, (typeof customers)[number]]> = customers
      .map((c) => [normalizeCustomerName(c.name || c.displayName || ""), c] as [string, (typeof customers)[number]])
      .filter(([key]) => Boolean(key));
    const existingMap = new Map(existingEntries);
    const resolvedLines = await Promise.all(draft.lines.map(async (line) => {
      const name = (line.customerId || "").trim();
      if (!name) return line;
      const normalized = normalizeCustomerName(name);
      const existing = existingMap.get(normalized);
      if (existing) return { ...line, customerId: existing.id, customerSnapshot: { id: existing.id, name: existing.name, code: existing.customerCode } };
      const created = await customersService.upsertCustomer?.({ id: createCustomerIdFromName(name), customerCode: `CUS-${Math.floor(Math.random() * 9000 + 1000)}`, name, displayName: name, normalizedName: normalized, source: "order-line", status: "active", totalOrders: 0, totalSpent: 0, outstandingAmount: 0, totalReceived: 0, storeCreditBalance: 0, totalReceivableGenerated: 0, currentReceivable: 0, createdAt: now, updatedAt: now } as any);
      if (created) existingMap.set(normalized, created);
      return created ? { ...line, customerId: created.id, customerSnapshot: { id: created.id, name: created.name, code: created.customerCode } } : line;
    }));
        ...
        <section className="card p-4">
          ...
          <div className="space-y-2">{history.length === 0 ? <div className="text-[12px] text-fg-subtle">No orders yet. Click Add Order to create one.</div> : history.map((o) => (
            <div key={o.id} className="rounded border border-border p-3 flex items-center justify-between gap-2">
              <div className="text-[12px]"><div className="font-semibold">{o.number}</div><div className="text-fg-subtle">{formatDate(o.date)} · Loading {o.loadingDate ? formatDate(o.loadingDate) : "—"} · {paymentAgents.find((p) => p.id === o.paymentBy)?.name ?? "—"} · {o.wechatId || "—"}</div><div className="text-fg-subtle">{o.lines.length} lines · {formatAmount(orderTotal(o))}</div></div>
