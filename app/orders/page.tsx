...
type HoverPreview = { key: string; anchor: HTMLElement | null; lines: Array<{ line: any; idx: number }> } | null;
  ...
  const getLineTooltip = (line: any, idx: number) => {
    const pcs = (Number(line.totalCtns) || 0) * (Number(line.pcsPerCtn) || 0);
    const ctns = Number(line.totalCtns) || 0;
    const qty = pcs > 0 || ctns > 0 ? `${pcs.toLocaleString()} PCS / ${ctns.toLocaleString()} CTNS` : "Not present";
    return `Line ${idx + 1} • Marka: ${line.marka || "Not present"} • Details: ${line.details || "Not present"} • Qty: ${qty}`;
  };
  ...
  return (
    <div className="flex h-screen min-h-0 flex-col">
                  ...
                  return <tr key={o.id} className="border-t border-border/80 hover:bg-bg-subtle/40 align-middle h-[68px]">
                    ...
                    <td>
                      <div className="flex items-center gap-1.5">
                        {primary.map(({ line, idx, image }) => <button key={line.id || idx} type="button" onMouseEnter={(e) => setHoverPreview({ key: `${o.id}-${idx}`, anchor: e.currentTarget, lines: [{ line, idx }] })} onMouseLeave={() => setHoverPreview((prev) => prev?.key === `${o.id}-${idx}` ? null : prev)} onFocus={(e) => setHoverPreview({ key: `${o.id}-${idx}`, anchor: e.currentTarget, lines: [{ line, idx }] })} onBlur={() => setHoverPreview((prev) => prev?.key === `${o.id}-${idx}` ? null : prev)} className="grid h-10 w-10 place-items-center overflow-hidden rounded-lg border border-border bg-bg-subtle">{image ? <img src={getCloudinaryOptimizedUrl(image, { width: 96, height: 96, crop: "fill" })} alt="product" className="h-full w-full object-cover" loading="lazy" decoding="async" /> : <span className="text-[10px] text-fg-subtle">—</span>}</button>)}
                        {more.length > 0 ? <button type="button" onMouseEnter={(e) => setHoverPreview({ key: `${o.id}-more`, anchor: e.currentTarget, lines: more.map(({ line, idx }) => ({ line, idx: idx + 3 })) })} onMouseLeave={() => setHoverPreview((prev) => prev?.key === `${o.id}-more` ? null : prev)} onFocus={(e) => setHoverPreview({ key: `${o.id}-more`, anchor: e.currentTarget, lines: more.map(({ line, idx }) => ({ line, idx: idx + 3 })) })} onBlur={() => setHoverPreview((prev) => prev?.key === `${o.id}-more` ? null : prev)} className="rounded-full border border-border bg-bg-subtle px-2 py-1 text-[11px] text-fg-muted">See more +{more.length}</button> : null}
                      </div>
                    </td>
      ...
      <FloatingPortal anchorRef={{ current: hoverPreview?.anchor as any }} open={Boolean(hoverPreview?.anchor)} width={280}>
        <div className="rounded-xl border border-border bg-bg-card p-2 shadow-card">
          <div className="space-y-2 text-[12px]">
            {(hoverPreview?.lines || []).map(({ line, idx }) => {
              const product = getHistoryLineImage(line);
              const dim = line.photoUrl || "";
              const ctns = Number(line.totalCtns) || 0;
              ...
              return <div key={`${line.id}-${idx}`} className="rounded-lg border border-border p-2">
                <div className="mb-1 flex items-center gap-2">
                  <div className="h-12 w-12 overflow-hidden rounded border border-border bg-bg-subtle">{product ? <img src={getCloudinaryOptimizedUrl(product, { width: 96, height: 96, crop: "fill" })} alt="product" className="h-full w-full object-cover" loading="lazy" decoding="async" /> : <div className="grid h-full place-items-center text-[10px] text-fg-subtle">Not present</div>}</div>
                  <div className="h-12 w-12 overflow-hidden rounded border border-border bg-bg-subtle">{dim ? <img src={getCloudinaryOptimizedUrl(dim, { width: 96, height: 96, crop: "fill" })} alt="dimension" className="h-full w-full object-cover" loading="lazy" decoding="async" /> : <div className="grid h-full place-items-center text-[10px] text-fg-subtle">Not present</div>}</div>
                </div>
                <div>Line {idx + 1}</div><div>Marka: {line.marka || "Not present"}</div><div>Details: {line.details || "Not present"}</div><div>CTNS: {ctns || "Not present"}</div><div>PCS / CTN: {pcsPer || "Not present"}</div><div>TOTAL PCS: {totalPcs || "Not present"}</div><div>rate / pcs: {rate || "Not present"}</div><div>Total amount: {totalAmount ? formatPlainAmount(totalAmount) : "Not present"}</div>
              </div>;
            })}
          </div>
        </div>
