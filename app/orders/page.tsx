...
export default function OrdersPage() {
  ...
  const getPaymentAgentName = (order: Order) =>
    ...
    || order.paymentBy
    || "Deleted payment agent";
  const getTotalCtns = (order: Order) => order.lines.reduce((sum, line) => sum + (Number(line.totalCtns) || 0), 0);
  const getOrderSecondarySummary = (order: Order) => {
    const supplierNames = Array.from(new Set(order.lines.map((l) => (l.supplierName || l.supplierSnapshot?.name || "").trim()).filter(Boolean)));
    if (supplierNames.length === 0) return "No supplier context";
    return supplierNames.length === 1 ? supplierNames[0] : `${supplierNames[0]} & ${supplierNames.length - 1} more`;
  };
            ...
            <table className="w-full min-w-[1080px] text-[13px]">
              ...
              <tbody>
                {history.length === 0 ? <tr><td colSpan={7} className="px-4 py-8 text-center text-fg-subtle">No orders yet. Click Add Order to create one.</td></tr> : history.map((o) => {
                  ...
                  const paymentName = getPaymentAgentName(o);
                  return <tr key={o.id} className="border-t border-border/80 hover:bg-bg-subtle/40 align-middle">
                    <td className="px-4 py-3"><div className="font-semibold">{o.number || o.orderNumber || "Draft"}</div><div className="text-[11px] text-fg-subtle truncate max-w-[250px]">{getOrderSecondarySummary(o)}</div></td>
                    <td><span className="inline-flex rounded-full border border-border bg-bg-subtle px-2.5 py-1 text-[11.5px]">{productCount} {productCount === 1 ? "Item" : "Items"}</span></td>
                    <td className="font-semibold text-[var(--success)] tabular-nums">{formatPlainAmount(orderTotal(o))}</td>
                    <td><div className="text-[12.5px]">{paymentName}</div>{o.paymentAgentSnapshot?.name && o.paymentAgentSnapshot?.name !== paymentName ? <div className="text-[11px] text-fg-subtle">{o.paymentAgentSnapshot.name}</div> : null}</td>
                    <td><input type="date" className="input h-8 text-[12px] w-[138px] rounded-full px-3" value={o.loadingDate ?? ""} onChange={(e) => { const updated = { ...o, loadingDate: e.target.value, updatedAt: new Date().toISOString() }; if (isFirebaseOrdersMode) { upsertFirebaseOrder(updated as any).then(reloadFirebaseOrders); } else { upsertOrder(updated); } }} /></td>
                    <td>{o.status !== "draft" && o.status !== "archived" ? <select className="input h-8 text-[12px] w-[118px] rounded-full px-3" value={o.status} disabled={statusUpdatingId === o.id} onChange={(e) => changeOrderStatus(o, e.target.value as Order["status"])}><option value="saved">saved</option><option value="loading">loading</option><option value="shipped">shipped</option><option value="received">received</option><option value="completed">completed</option><option value="cancelled">cancelled</option></select> : <span className="inline-flex rounded-full border border-border bg-bg-subtle px-2 py-1 text-[11px] text-fg-subtle">{o.status}</span>}</td>
