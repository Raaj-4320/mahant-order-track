              ...
              <tbody>
                {history.length === 0 ? <tr><td colSpan={7} className="px-4 py-8 text-center text-fg-subtle">No orders yet. Click Add Order to create one.</td></tr> : history.map((o) => {
                  ...
                  return <tr key={o.id} className="border-t border-border/80 hover:bg-bg-subtle/40 align-middle h-[68px]">
                    <td className="px-4 py-3"><div className="font-semibold">{o.number || o.orderNumber || "Draft"}</div><div className="text-[11px] text-fg-subtle truncate max-w-[170px]">{fmtDateTime(o)}</div></td>
                    <td>
                      <div className="flex items-center gap-1.5">
                        {primary.map(({ line, idx, image }) => <div key={line.id || idx} title={getLineTooltip(line, idx)} className="grid h-10 w-10 place-items-center overflow-hidden rounded-lg border border-border bg-bg-subtle">{image ? <img src={getCloudinaryOptimizedUrl(image, { width: 96, height: 96, crop: "fill" })} alt="product" className="h-full w-full object-cover" loading="lazy" decoding="async" /> : <span className="text-[10px] text-fg-subtle">—</span>}</div>)}
                        {more.length > 0 ? <span className="rounded-full border border-border bg-bg-subtle px-2 py-1 text-[11px] text-fg-muted" title={more.map(({ line, idx }) => getLineTooltip(line, idx + 3)).join(" | ")}>See more +{more.length}</span> : null}
                      </div>
                    </td>
                    ...
                    <td><div className={paymentMeta.isMissing ? "text-[var(--danger)] text-[12.5px]" : "text-[12.5px]"}>{paymentName}</div>{o.paymentAgentSnapshot?.name && o.paymentAgentSnapshot?.name !== paymentName ? <div className="text-[11px] text-fg-subtle">{o.paymentAgentSnapshot.name}</div> : null}</td>
