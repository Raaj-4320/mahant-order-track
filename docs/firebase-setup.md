...
Firebase mode never mixes demo fixture data into real runtime. Dev seed is an optional future tool and is not automatic.

## Firestore rules hardening (Phase 6G)
- Use `firestore.rules` in this repo as the baseline secure draft.
- Requires Firebase Auth + business member docs before deployment.
- Dev reset (`NEXT_PUBLIC_ENABLE_DEV_RESET=true`) still requires owner/admin delete permissions in rules.
- Do not deploy permissive rules (`allow read, write: if true`).

## Auth + bootstrap (Phase 6H)
- Enable Firebase Auth (Email/Password or preferred provider).
- Use Sidebar Auth panel to sign in/sign up and run owner membership bootstrap.
- Bootstrap creates/updates `businesses/{businessId}` and `businesses/{businessId}/members/{uid}` owner member record.
- Deploy `firestore.rules` manually only after auth + membership bootstrap is ready.

- Customer receivable ledger collection used in Phase 8D.2: businesses/{businessId}/customerLedger.

- Customer payments in Phase 8D.3 are posted via Firebase customers service and written to customerLedger as customer_payment rows.